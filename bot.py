...
async def story_menu_kb(story_id: int) -> InlineKeyboardMarkup:
    ...
    buttons = []
    # Проверяем наличие иллюстраций
    illustr_path = Path(__file__).parent / "illustraciones" / story['rus_title']
    has_illustrations = illustr_path.exists() and any(illustr_path.iterdir())
    
    # Кнопка иллюстраций (если есть файлы)
